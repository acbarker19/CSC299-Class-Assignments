...
class BoidUniverse:
   ...
   def animate(self):
      ...
      # we only want one boid to make noise at a time  
      self.counter = 0
      
      # set the pitch based on the x coordinate of a single boid
      pitch = mapScale(self.boids[self.counter].coordinates.real, 0, 1000, self.lowPitch, self.highPitch, self.scale)
      dynamic = randint(0, 127)
      Play.note(pitch, 0, self.noteDuration, dynamic)
      
      # add 1 to the counter every frame
      # if the counter is larger than the number of boids, it loops back to zero
      self.counter += 1
      if self.counter > len(self.boids):
         self.counter = 0
...
universe = BoidUniverse(title="Boid Flocking Behavior", width=universeWidth, height=universeHeight, frameRate=30)
