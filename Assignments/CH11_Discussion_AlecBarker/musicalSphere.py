...
class MusicalSphere:
   ...
   def __init__(self, radius, density, velocity=0.01, frameRate=30):
      """ Construct a revolving sphere with given 'radius', 'density' number of points (all on the surface), moving with 'velocity' angular (theta / azimuthal) velocity, at 'frameRate' frames (or movements) per second. Each point plays a note when crossing the zero meridian (the sphere's meridian (vertical line) closest to the viewer). """
      ...
      self.lowPitch = C1
      self.highPitch = C4
      self.noteDuration = 20 # milliseconds
      ...
      # creates the variables to set up the animation based off of the parameters that the user puts in the function
      ...
      # adds a slider to control the frame rate
      xPosition = self.display.getWidth() / 3 # position control
      yPosition = self.display.getHeight() + 45
      self.control = SliderControl(title="Frame Rate", updateFunction=self.setFrameRate, minValue=1, maxValue=120, startValue=self.frameRate, x=xPosition, y=yPosition)

      ### color gradient (used to display depth) ###################
      # the changing colors of the points
      ...
      # create list of gradient colors from black to orange, and from orange to white (a total of 25 colors)
      self.gradientColors = colorGradient(black, orange, 12) + colorGradient(orange, white, 12) + [white] # include the final color 
   ...
   def initSphere(self):
      ...
      for i in range(self.numPoints): # create all the points
         ...
         # remember this point's spherical coordinates by appending them to the two parallel lists (since r = self.radius for all points, no need to save that)
         ...
         # create point at these x, y coordinates, with this color and thickness 1
   ...
   def movePoints(self):
      ...
      for i in range(self.numPoints): # for every point
         ...
         # wrap around at the primary meridian (i.e., 360 degrees become 0 degrees) - needed to decide when to play note
         theta = theta % (2*pi)

         # convert from spherical to cartesian 2D coordinates
         x, y, z = self.sphericalToCartesian(self.radius, phi, theta)

         # check if point crossed the primary meridian (0 degrees), and if so, play musical note, and change its color to red
